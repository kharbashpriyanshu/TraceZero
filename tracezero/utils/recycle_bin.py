...
class RecycleBinManager:
    ...
    def delete_items(self, items: List[Dict]) -> Tuple[List[str], List[str]]:
        ...
        for item in items:
            ...
            # ── Resolve Windows 8.3 short paths (e.g. PRIYAN~1 → PRIYANSHU)
            try:
                import ctypes
                buf = ctypes.create_unicode_buffer(32768)
                get_long = ctypes.windll.kernel32.GetLongPathNameW
                if get_long(str(path), buf, 32768):
                    path = Path(buf.value)
                    path_str = str(path)
            except Exception:
                pass  # Not on Windows or ctypes unavailable — use path as-is
            ...
            try:
                ...
                err_str = str(e)
                # 0x80270027 = file in use / can't be recycled (common for locked temp files)
                if "80270027" in err_str or "in use" in err_str.lower():
                    app_logger.warning(f"⏭ Skipped (file in use by Windows): {path.name}")
                    failed.append(f"{path_str} (in use — skip)")
                else:
